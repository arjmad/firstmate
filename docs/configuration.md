...
A backend close that reaches a live endpoint and fails stops teardown loudly with the task's worktree, state files, and temp roots preserved intact for a rerun; `bin/fm-teardown.sh`'s header owns the exact per-backend close and continuation mechanics.
...
The local `config/herdr-presentation-spaces` file instead opts a home in to, or explicitly out of, Herdr's disposable single-task visual projection, which stays off for an unconfigured home; [Presentation spaces](herdr-backend.md#presentation-spaces) owns its accepted values, default, Herdr version floor, migration, behavior, safety limits, recovery contract, and narrow locked session-start cleanup of exact restored idle-shell children.
...
prime-agent is an EXPERIMENTAL crewmate and scout adapter, selected only when asked for explicitly and never as a dispatch default; `fm-spawn.sh` refuses it for a secondmate for the same reason as muse.
It also has no verified way to stop a running agent - its Ctrl-D detaches the client while the daemon and its Python worker keep running - so lifecycle control offers it `interrupt` only, and `exit`, `relaunch`, and `--relaunch` are all refused rather than reporting a stop that did not happen.
Each of its tasks gets a private short-path socket root and session directory, recorded in that task's own record, and cleanup stops the recorded session before shutting down only that task's daemon.
...
That delivery is per-backend.
On the herdr backend the launch environment is placed on the pane process natively at window creation through herdr's `tab create --env`, so the token never transits the pane's interactive shell; it is passed as an argument to the herdr client at pane creation and is briefly visible in a local process listing, a deliberate tradeoff against the larger exposure of the credential on the pane's visible screen and in Herdr's persisted pane history.
On every backend without that native placement, and on a herdr relaunch (which creates no window), the token is exported into the crewmate's pane shell immediately before launch through the typed pre-launch channel that ships `GOTMPDIR` there, with that shell's history file suppressed for the line so the secret is not persisted to disk by the shell.
...
See [`docs/examples/model-endpoints.json`](examples/model-endpoints.json) for a starting point to copy into local `config/model-endpoints.json`.
Resolution, launch composition, credential handling, and the fail-closed cases are covered by `tests/fm-model-endpoint.test.sh` and `tests/fm-spawn-model-endpoint.test.sh`.
...
FM_LOCK_STEAL_TOKEN_ATTEMPTS=3   # times one stale-lock takeover may recover an abandoned steal token before giving up so the caller retries the acquisition
