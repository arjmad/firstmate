...
| `fm-registry-snapshot.sh` | Emit the versioned bounded Registry operational contract (schema `fm-registry-snapshot.v1`), local-only except remote secondmate rows read over SSH |
...
| `fm-model-endpoint.sh`   | Resolve a claude `--model` against `config/model-endpoints.json` to a local proxy endpoint, fail-closed |
...
| `fm-prime-agent.sh`      | Task-scoped Prime Agent control adapter: launch, session discovery, authoritative state, steering, and scoped cleanup |
