...
| `fm-herdr-lab.sh`        | Provision, guardedly operate, and durably reclaim isolated, never-default Herdr lab sessions |
...
| `fm-spawn.sh`            | Spawn or relaunch crewmates and scouts, dispatch `id=repo` batches, and spawn secondmates on the resolved harness and runtime backend |
...
| `fm-tangle-lib.sh`       | Shared primary-checkout and default-branch resolution, plus tangle classification    |
...
| `fm-prime-agent.sh`      | Task-scoped Prime Agent control adapter owning launch, session discovery, authoritative state, steer send, shutdown, and cleanup |
