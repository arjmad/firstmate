...
It classifies recognized Claude, Codex, OpenCode, Pi, pi-signed, Grok, Kimi, Muse, and prime-agent process names as `alive`, common shells as `dead`, an authoritatively absent window as `missing`, unreadable state as `unreadable`, and every other process as `ambiguous`.
...
A third, narrowest source reads only argv[1] of a recognized bare interpreter (node, python) in the foreground group, because an adapter whose published entrypoint is an interpreted script - prime-agent's npm entrypoint runs as `node` - exposes the harness name in neither `comm` nor argv[0].
Any one source naming a verified harness is enough for `alive`, because a false `dead` is the one verdict that can start a duplicate agent on a live worktree, while a readable foreground process group settles the negative verdicts.
