...
The idle-shell and pane-death proofs require a `ps` that reports the process state letter, because each accepts only a sleeping or idle shell, and the process-level proof above reads the same `ps`.
