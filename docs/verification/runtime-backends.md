...
Opt-in presentation projection is fully focus-safe only from its own floor at Herdr 0.8.0, protocol 19, verified below.
...
The projection suite ran again on 2026-08-04 against Herdr 0.8.0 protocol 19 under upstream's then-current default-on behavior, where an absent `config/herdr-presentation-spaces` enabled the projection and the value `off` opted out.
The projection has since been made opt-in - an absent file keeps the flat layout on every release, with [Presentation spaces](../herdr-backend.md#presentation-spaces) owning the current default - so the unconfigured-home line below is historical evidence from that superseded default:
...
One concurrent cross-home recovery case refused under contention on a loaded machine and passed on an immediate rerun; recovery-path presentation lock contention is a deliberate hard refusal rather than a flat fallback, reachable only in a home that enabled the projection or still holds a journal from when it had.
That run measured the then-default projection on Herdr 0.8.0 only, while the focus-flash regression below was last run on 0.7.5, so neither run covered a defective release with the projection active; under the opt-in default that combination needs an explicit `on`, and the version floor and the focus-flash suite's Part C close that gap.
...
Both recorded runs predate the opt-in default: the suite's final case now asserts on every release that an unconfigured home stays flat while the explicit opt-in still projects, replacing the two release-dependent `version floor:` unconfigured-home lines above, and has not yet been re-run live.
...
On 0.7.5 that fallback exposed a bounded four-sample wrong-focus window and restored the anchor exactly; on 0.8.0 the same fallback exposed none, which is why the projection is fully focus-safe only from the 0.8.0 floor rather than mitigated further below it.
...
Presentation projection is fully focus-safe from Herdr 0.8.0, the version floor.
Under the opt-in default an unconfigured home never consults the floor and always stays flat, and an explicit `on` is honored below the floor, accepting the documented focus move; the floor's own classifier and warn-once behavior remain owned by `fm_backend_herdr_presentation_default_supported`.
...
That ordinary post-upgrade shape proves the running server owns the focus behavior, so the floor owner composes client and selected-server verdicts conservatively and rechecks after server ensure before publishing a journal or creating a workspace.
...
Both runs reported `family=real-herdr-gated count=11 failed=0` and predate the opt-in default.
The projection suite's unconfigured-home case was release-aware rather than pinned to one outcome, so those runs proved the then-current projected default on 0.8.0 and the flat fallback with its naming warning on 0.7.4:
