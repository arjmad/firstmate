...
A refused fresh spawn leaves no worker endpoint behind: origin reachability is proved on the project before any endpoint exists, and an endpoint created before the task record is published is closed when the spawn exits without publishing it, so a retry of the same task id is never blocked by the previous attempt's debris.
