...
Pooled worktree and secondmate-home paths are reused, so before any of that runs, both spawn and teardown prove no other current task record claims the same allocation and refuse loudly when one does; [`bin/fm-allocation-lib.sh`](../bin/fm-allocation-lib.sh) owns that predicate, why a matching path, a pane label, a dead endpoint, or a task-ID lease is not ownership, and why `--force` does not lift it.
