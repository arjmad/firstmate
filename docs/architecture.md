...
`bin/fm-fleet-view.sh` renders that snapshot as Markdown for humans, `bin/fm-bearings-snapshot.sh` provides the bounded bearings projection, and `bin/fm-registry-snapshot.sh` reuses it for the local-only `fm-registry-snapshot.v1` Registry contract, so all three consumers share one structured contract instead of reparsing raw fleet files.
...
That emitted block owns the live wait shape, while the `harness-adapters` skill owns the verified harness facts that select it.
...
On `attached` it stays live across identity-matched successors, and an unexplained clean child close either attaches to a verified healthy successor or becomes the typed nonzero `watcher: FAILED - cycle ended without an actionable reason` result; the one exception is an attached follower whose cycle ledger proves the owner arm already delivered that cycle's actionable wake, which closes quietly ([watcher-continuity.md](watcher-continuity.md) owns the arm-layer cycle contract).
...
The [`/afk` skill](../.agents/skills/afk/SKILL.md) is the single owner of entry, sentinel interpretation, classification, batching, approval boundaries, and ordered return.
The daemon and watcher share `bin/fm-classify-lib.sh`, while `bin/fm-backend.sh` dispatches supervisor-pane capture, busy, composer, and submit operations.
The tmux busy-queued Enter exception is owned beside `fm_tmux_submit_enter_core` in `bin/fm-tmux-lib.sh`; the Herdr-specific gap remains recorded in [`herdr-backend.md`](herdr-backend.md).
Composer-content classification has one shared owner, `bin/fm-composer-lib.sh`, used after each backend identifies its composer row.
The wedge-alarm configuration schema is owned by [`configuration.md`](configuration.md#away-mode-wedge-alarm-channels-configwedge-alarm), and [`wedge-alarm.md`](wedge-alarm.md) retains the rationale and verification evidence.
...
`bin/fm-backend.sh`'s header is the single owner of selection precedence, configuration parsing, runtime marker order, notices, validation, and fallback.
[`configuration.md`](configuration.md#runtime-backend-configbackend--fm_backend) owns the accepted configuration values.
...
Herdr is experimental; selection follows the owner in `bin/fm-backend.sh`, treehouse remains its worktree provider, and its verification record and known gaps live in [`herdr-backend.md`](herdr-backend.md).
...
Its opt-in `fm-spawn.sh --fleet` flag can instead group related spawns as pane splits in one dedicated grid workspace, likewise without changing endpoint authority; [`docs/herdr-backend.md`](herdr-backend.md#opt-in-fleet-grid-workspaces) owns that conditional design.
Zellij is experimental; selection follows the owner in `bin/fm-backend.sh`, treehouse remains its worktree provider, and its verification record and known gaps live in [`zellij-backend.md`](zellij-backend.md).
...
Orca is experimental; selection follows the owner in `bin/fm-backend.sh`, Orca owns both worktree and terminal lifecycle, and its current behavior and limitations are recorded in [`orca-backend.md`](orca-backend.md).
cmux is experimental, GUI-first, and macOS-only; selection follows the owner in `bin/fm-backend.sh`, treehouse remains its worktree provider, and its verification record and known gaps live in [`cmux-backend.md`](cmux-backend.md).
...
That keeps spawn launch compatible across the verified adapters owned by `harness-adapters` while preserving the requested profile for later audit.

## Optional secondmates

`data/secondmates.md` records persistent routes under the line schema owned by the [`secondmate-provisioning` skill](../.agents/skills/secondmate-provisioning/SKILL.md#routing-table).
...
Secondmate agents can use a different verified harness from crewmates.
[`configuration.md`](configuration.md#harness-configuration) is the single owner of the `config/secondmate-harness` schema and launch precedence.
The [`secondmate-provisioning` skill](../.agents/skills/secondmate-provisioning/SKILL.md) owns the inherited-local-material set and its propagation consequences.
That skill's [routing-table section](../.agents/skills/secondmate-provisioning/SKILL.md#routing-table) also owns the `data/secondmates.md` line schema.
...
Fleet state lives in each task's selected session-provider backend, no-mistakes run records, status event logs, local markdown under `data/`, and persistent secondmate homes.
The backend-selection contract remains owned by `bin/fm-backend.sh`.
