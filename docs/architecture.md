...
A prime-agent crew without a run instead reconciles authoritative `list --json` state through `bin/fm-prime-agent.sh` before any pane or status-log inference, because spinner absence never proves Prime Agent idle; [`adapter-prime.md`](../.agents/skills/harness-adapters/references/adapter-prime.md) owns that contract.
...
Because this fallback re-reads the pane, it is also the only reader that can credit claude's persistent running-shell footer, and only by proving the matched footer line itself changed between two samples; the single-sample watcher and daemon readers deliberately cannot see that signature, so a footer frozen by a killed harness can never suppress their wedge recovery ([`.agents/skills/harness-adapters/references/adapter-claude.md`](../.agents/skills/harness-adapters/references/adapter-claude.md) owns the signature and its freshness boundary).
...
Allocation is the default; the opt-in `fm-spawn.sh --reuse-worktree <path>` flag instead relaunches one stopped ship or scout into a caller-selected existing worktree of the same project, so unlanded work stays with the task rather than being stranded beside a freshly allocated slot.
The same isolation refusal still applies, and the script's header owns the flag's exact preconditions and refusals.
...
Only a named non-default branch checked out in the primary is a worktree tangle.
`FM_ROOT` is just the checkout the fleet action runs from, which is itself a linked worktree on `fm/<id>` whenever a crewmate works on firstmate, so the guards resolve the repository's primary checkout from it instead of classifying it directly.

`fm-tangle-lib.sh` resolves the primary checkout (the repository's main worktree, or nothing when it is bare), resolves the default branch from `origin/HEAD`, then local `main` or `master`, and classifies that named non-default primary branch as the tangle.
The doctrine is scoped to the firstmate repo alone; an ordinary project's checkout may sit on any branch and is never inspected.
...
That keeps spawn launch compatible across every verified harness while preserving the requested profile for later audit.
...
`config/crew-dispatch.json` and `config/model-endpoints.json` are inherited too; secondmates use the same natural-language dispatch profiles and the same local model-endpoint routing when spawning their own crewmates.
