...
The same suite pins the lock primitives against not returning: a lock whose recorded holder is the acquiring process itself is reclaimed instead of waited on, an abandoned steal token is recovered in place without deepening the token chain, and an unusable steal-token path refuses the takeover promptly.
