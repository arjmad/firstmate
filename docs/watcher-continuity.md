...
Its required position before the synchronous turn-end guard is owned by [`turnend-guard.md`](turnend-guard.md#harness-integrations).
...
That stable identity lets a lock-command lane and a later Stop-hook lane in the same session agree after a mid-session takeover without letting an inherited value override a nested non-Claude harness or authorize a competing session.
...
`bin/fm-watch-arm.sh` never returns an unexplained clean empty success.
...
An attached follower whose chain ends without a successor first consults the cycle ledger for an `actionable-*` record naming its watcher PID inside the attach window; the owner arm writes that record before printing the wake.
On a recorded delivery the follower appends an `attached-cycle-delivered` row and closes quietly (exit 0, no output) so one wake is never notified twice; otherwise it reports the same typed failure.
A killed or crashed watcher writes no actionable record, so genuine silent death still fails loud, and the owner arm's own empty-close failure is unchanged.
...
`tests/fm-claude-stop-autoarm.test.sh` covers the auto-arm's scope, stale and live session owners, mid-session takeover across distinct Claude execution lanes, ordinary-cycle re-claim in registered Stop-hook order, unchanged AFK and need boundaries, single-flight, and exit-2 translation.
...
[`verification/supervision.md`](verification/supervision.md#watcher-continuity) records the current five-harness live evidence, the Stop-owned Claude auto-arm results, and exact opt-in commands.
