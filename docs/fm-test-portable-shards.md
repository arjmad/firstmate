...
This fork adds its own measurement of `tests/fm-google-workspace.test.sh` from run [34058005006](https://github.com/arjmad/firstmate/actions/runs/34058005006) on 2026-09-06 and a provisional 30000 ms hint for `tests/fm-state-residue-sweep.test.sh` scaled from a 2026-09-17 local measurement of 10757 ms until a green fork run measures it.
The fork is a private repository, so its `ubuntu-latest` jobs run on the 2-vCPU hosted runner rather than the 4-vCPU runner upstream's public repository gets, and its scripts have measured up to 2.5x slower than the upstream runs; the shared 30-minute Normal-tier bound still leaves hang-tripwire margin over the packed estimate at that spread.
...
`bin/fm-lint.sh` owns the canonical CI partitions, each running the same full source-aware ShellCheck analysis with two bounded workers, pinned versions, workflow validation, and backend-purity checks; upstream runs two, and this fork's `ci.yml` selects eight because its smaller private-repository runner cannot hold the heaviest source graphs in one job (the lint job's own comment owns that evidence).
