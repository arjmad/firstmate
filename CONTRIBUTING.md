...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `.claude/settings.json`, `bin/`, `.agents/skills/`, and `skills/`.
  ...
  The private fleet surface defined by [`docs/configuration.md`](docs/configuration.md#operational-home-layout-and-state) is gitignored; never commit it.
  ...
  Runtime-backend configuration is owned by [`docs/configuration.md`](docs/configuration.md), and exact selection precedence is owned by the `bin/fm-backend.sh` header.
  It does not make `data/` tracked.
- Helper scripts in `bin/` are plain bash.
  Each starts with a usage header comment; keep it accurate when you change behavior.
  Shell test scripts and helpers in `tests/` are plain bash; Python behavior tests use the standard library's `unittest` runner.
  Shell behavior tests must keep their executable bit: CI invokes them directly and fails on a non-executable test (enforced by the CI guard and pinned by `tests/fm-test-runners.test.sh`).
...
Tracked changes to firstmate itself - `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `.claude/settings.json`, `bin/`, `.agents/skills/`, and `skills/` - ship through the `no-mistakes` pipeline on a feature branch and require an explicit merge approval.
...
for test_script in tests/*.test.sh; do bash "$test_script"; done   # shell behavior tests, matching CI and no-mistakes commands.test
for test_script in tests/*.test.py; do python3 "$test_script"; done   # Python behavior tests, matching CI and no-mistakes commands.test
[ "$(readlink CLAUDE.md)" = "AGENTS.md" ]
[ "$(readlink .claude/skills)" = "../.agents/skills" ]
tmp=$(mktemp -d) && printf 'done: smoke\n' > "$tmp/smoke.status" && FM_STATE_OVERRIDE="$tmp" FM_SIGNAL_GRACE=1 FM_POLL=1 FM_HEARTBEAT=999999 bin/fm-watch-arm.sh  # watcher re-arm smoke test (prints arm status, then an actionable signal)
```

Discover tests by listing `tests/*.test.sh` and `tests/*.test.py`.
Each is a self-contained script named `<subject>.test.sh` or `<subject>.test.py`, and its header comment or module contents describe what it covers, so run one directly to focus on a subject.
Tests that need a real optional backend or an explicit opt-in (real herdr/zellij/cmux smoke tests, the live Pi regression) skip themselves and print the tool or environment gate needed to enable them, so the run-all loops above are always safe.
