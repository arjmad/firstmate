...
# worktree write probe, the process probe, and the dead-record probe
# (wedge_dead_record) run ONLY here, inside the at-threshold branch that is about
# to escalate: at most one each per window per STALE_ESCALATE_SECS, never per
# poll. The wait consult runs first, because a pane whose worker already said why
# it is quiet has nothing to prove through its worktree; the write probe goes
# before the process probe because it is the cheaper of the two, and any one
# alone defers. The dead-record probe runs last, so the cheaper deferrals keep
# the panes they already own on their existing bounded cadences and only a pane
# that would otherwise alarm pays for a backend read.
