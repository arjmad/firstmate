...
The main firstmate's answer normally writes that closing line at answer time; when a blocker or wait clears WITHOUT an answer from the main firstmate, append \`resolved [key=<slug>]: {how it cleared}\` yourself as your domain resumes, reusing the slug you opened it with (drop the \`[key=...]\` entirely if you opened it without one). State the key BEFORE the colon: a \`[key=...]\` written later in the note is read as prose and closes nothing.
...
6. If a decision belongs to a human (product choices, destructive actions),
   ...
   Firstmate's reply normally writes that closing line at answer time; when a blocker or wait clears WITHOUT a firstmate reply, append \`resolved [key=<slug>]: {how it cleared}\` yourself as you resume, reusing the slug you opened it with (drop the \`[key=...]\` entirely if you opened it without one). State the key BEFORE the colon: a \`[key=...]\` written later in the note is read as prose and closes nothing.
