...
#   A fresh ship or scout with a detected origin first proves that origin is reachable, before
...
[ "$FAST_SET" -eq 0 ] || [ -n "$FAST" ] || {
  echo "error: --fast requires a non-empty value" >&2
  exit 1
}
...
case "$FAST" in
  on|off) ;;
  *)
    echo "error: --fast accepts on or off, not '$FAST'" >&2
    exit 1
    ;;
esac
...
launch_template() {
  ...
  codex)
    if [ "$kind" = secondmate ]; then
      ...
      printf '%s' 'codex __MODELFLAG____EFFORTFLAG____TIERFLAG__--dangerously-bypass-approvals-and-sandbox --disable hooks -c "notify=[\"bash\",\"-c\",\"touch __TURNEND__\"]" "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
    fi
    ;;
...
#
# A project with no origin configuration is skipped here exactly as the gate
# skips its remote freshness check for it (spawn_worktree_has_origin_config
# below), so a pooled spawn without a git origin is still allowed.
...
if [ "$KIND" != secondmate ]; then
  ...
  claude*)
    ...
    # the turn-ended NOTIFICATION touch for the watcher. PostToolUse refreshes
    # the separate generation-bound progress marker (fm-busy-event.sh progress)
    # after every completed tool call, so a worker busy inside a long monitored
    # gate keeps the watcher's busy-age bound fresh without fabricating a
    # completed turn; Claude Code 2.1.274 has no periodic event during a
    # long-running or background tool, so the completed-call boundary is the
    # finest native activity signal available. Every hook command tolerates a
    # refused event (|| true) so a stale-gen writer can never break Claude's
    # own lifecycle, and PostToolUse cannot block a call in any case.
    mkdir -p "$WT/.claude"
    ...
    j_posttool=$(json_escape "$(shell_quote "$FM_ROOT/bin/fm-busy-event.sh") progress $(shell_quote "$STATE_REAL") $(shell_quote "$ID") --gen $(shell_quote "$BUSY_GEN") >/dev/null 2>&1 || true")
    cat >"$WT/.claude/settings.local.json" <<EOF
{"hooks":{"UserPromptSubmit":[{"hooks":[{"type":"command","command":"$j_submit"}]}],"PostToolUse":[{"hooks":[{"type":"command","command":"$j_posttool"}]}],"Stop":[{"hooks":[{"type":"command","command":"$j_stop"}]}],"StopFailure":[{"hooks":[{"type":"command","command":"$j_stopfail"}]}],"SessionEnd":[{"hooks":[{"type":"command","command":"$j_sessionend"}]}]}}
EOF
