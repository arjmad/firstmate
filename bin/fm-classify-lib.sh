...
# Both declarations mean the crew's idle endpoint is EXPECTED - whether its
# agent is still alive (a parked crew holding for an external decision) or has
# exited - so the watcher applies its bounded pause cadence instead of
# re-surfacing the pane, and relies on status movement and authoritative
# working evidence to end the hold.
