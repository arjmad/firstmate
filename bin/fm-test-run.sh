...
family_for_basename() {
  case "$1" in
    ...
    fm-kimi-harness.test.sh|fm-muse-harness.test.sh|fm-prime-agent-harness.test.sh|\
    ...
    fm-spawn-model-endpoint.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-test-run.sh|bin/fm-test-isolation-proof.sh)
      printf '%s\n' pure-contract-unit
      ;;
    bin/backends/herdr*|bin/fm-herdr-lab.sh|bin/fm-herdr-env-lib.sh|tests/herdr-test-safety.sh)
      printf '%s\n' real-herdr-gated
      printf '%s\n' backend-dispatch
      printf '%s\n' pure-contract-unit
      ;;
