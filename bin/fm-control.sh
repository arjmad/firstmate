...
#              the agent gone. Already-stopped is success (idempotent). An
#              endpoint that reads `missing` is put through the control plane's
#              per-backend absence proof (fm_control_endpoint_absence_verdict)
#              before anything is claimed about it, because `missing` also
#              covers an endpoint that is merely unreachable from this seat. That proof exists only on
...
do_exit() {
  local state cmd hazard verdict composer_state cancel confirm absence interrupt_result=not-needed
