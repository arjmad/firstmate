...
# Opt-in fleet grids use one durable record per path-safe fleet name.
...
fm_backend_herdr_fleet_record_path() {  # <state-dir> <fleet-name>
...
# Publish a pending record before the first workspace mutation.
# A pending, malformed, or ambiguous record never authorizes reuse or cleanup.
fm_backend_herdr_fleet_record_create_pending() {  # <state-dir> <fleet-name>
  local state=$1 fleet=$2 record token tmp
  case "$fleet" in
    ''|.*|*[!A-Za-z0-9._-]*)
      echo "error: invalid Herdr fleet name" >&2
      return 1
      ;;
  esac
  [ "${#fleet}" -le 64 ] || return 1
  mkdir -p "$state" || return 1
  record=$(fm_backend_herdr_fleet_record_path "$state" "$fleet")
  if [ -e "$record" ] || [ -L "$record" ]; then
    echo "warning: Herdr fleet record already exists for '$fleet'; refusing to adopt or overwrite it" >&2
    return 2
  fi
  ...
  if ! ln "$tmp" "$record" 2>/dev/null; then
    rm -f "$tmp"
    echo "warning: Herdr fleet record appeared concurrently for '$fleet'; refusing fleet workspace creation" >&2
    return 2
  fi
  rm -f "$tmp"
  printf '%s' "$token"
...
fm_backend_herdr_fleet_record_pending_matches() {  # <record> <fleet-name> <fleet-id>
  local record=$1 fleet=$2 token=$3 lines
  [ -f "$record" ] && [ ! -L "$record" ] || return 1
  lines=$(wc -l < "$record" 2>/dev/null | tr -d '[:space:]')
  [ "$lines" = 4 ] || return 1
  [ "$(grep '^version=' "$record" | cut -d= -f2- || true)" = 1 ] || return 1
  [ "$(grep '^fleet=' "$record" | cut -d= -f2- || true)" = "$fleet" ] || return 1
  [ "$(grep '^fleet_id=' "$record" | cut -d= -f2- || true)" = "$token" ] || return 1
  [ "$(grep '^state=' "$record" | cut -d= -f2- || true)" = pending ]
}
...
fm_backend_herdr_fleet_record_activate() {  # <record> <fleet> <fleet-id> <session> <workspace-id> <tab-id>
  ...
  fm_backend_herdr_fleet_record_pending_matches "$record" "$fleet" "$token" || { rm -f "$tmp"; return 1; }
  mv "$tmp" "$record"
}

# Validate one active record without sourcing it as shell code.
# Success publishes response values in FM_BACKEND_HERDR_FLEET_RECORD_* globals.
fm_backend_herdr_fleet_record_read() {  # <record> <fleet-name>
  local record=$1 fleet=$2 lines version recorded_fleet token state session workspace tab
  FM_BACKEND_HERDR_FLEET_RECORD_TOKEN=""
  FM_BACKEND_HERDR_FLEET_RECORD_SESSION=""
  FM_BACKEND_HERDR_FLEET_RECORD_WORKSPACE_ID=""
  FM_BACKEND_HERDR_FLEET_RECORD_TAB_ID=""
  [ -f "$record" ] && [ ! -L "$record" ] || return 1
  lines=$(wc -l < "$record" 2>/dev/null | tr -d '[:space:]')
  [ "$lines" = 7 ] || return 1
  version=$(grep '^version=' "$record" | cut -d= -f2- || true)
  recorded_fleet=$(grep '^fleet=' "$record" | cut -d= -f2- || true)
  token=$(grep '^fleet_id=' "$record" | cut -d= -f2- || true)
  state=$(grep '^state=' "$record" | cut -d= -f2- || true)
  session=$(grep '^session=' "$record" | cut -d= -f2- || true)
  workspace=$(grep '^workspace_id=' "$record" | cut -d= -f2- || true)
  tab=$(grep '^tab_id=' "$record" | cut -d= -f2- || true)
  [ "$version" = 1 ] && [ "$recorded_fleet" = "$fleet" ] && [ "$state" = active ] || return 1
  [ "${#token}" -eq 22 ] && [ -n "$session" ] && [ -n "$workspace" ] && [ -n "$tab" ] || return 1
  case "$token$session$workspace$tab" in *[!A-Za-z0-9._:-]*) return 1 ;; esac
  FM_BACKEND_HERDR_FLEET_RECORD_TOKEN=$token
  FM_BACKEND_HERDR_FLEET_RECORD_SESSION=$session
  FM_BACKEND_HERDR_FLEET_RECORD_WORKSPACE_ID=$workspace
  FM_BACKEND_HERDR_FLEET_RECORD_TAB_ID=$tab
}
...
fm_backend_herdr_fleet_workspace_matches_record() {  # <session> <workspace-id> <tab-id> <fleet> <fleet-id>
  local session=$1 workspace=$2 tab=$3 fleet=$4 token=$5 workspace_out tab_out expected
  expected=$(fm_backend_herdr_fleet_workspace_label "$fleet" "$token")
  workspace_out=$(fm_backend_herdr_cli "$session" workspace get "$workspace" 2>/dev/null) || return 1
  printf '%s' "$workspace_out" | jq -e --arg workspace "$workspace" --arg expected "$expected" '
    .result.workspace.workspace_id == $workspace and .result.workspace.label == $expected
  ' >/dev/null 2>&1 || return 1
  tab_out=$(fm_backend_herdr_cli "$session" tab get "$tab" 2>/dev/null) || return 1
  printf '%s' "$tab_out" | jq -e --arg workspace "$workspace" --arg tab "$tab" '
    .result.tab.workspace_id == $workspace and .result.tab.tab_id == $tab
  ' >/dev/null 2>&1
}

fm_backend_herdr_fleet_create() {  # <state-dir> <fleet-name> <cwd>
  local state=$1 fleet=$2 cwd=$3 session record token label before out workspace tab pane
  FM_BACKEND_HERDR_FLEET_SESSION=""
  FM_BACKEND_HERDR_FLEET_WORKSPACE_ID=""
  FM_BACKEND_HERDR_FLEET_TAB_ID=""
  FM_BACKEND_HERDR_FLEET_PANE_ID=""
  session=$(fm_backend_herdr_session)
  record=$(fm_backend_herdr_fleet_record_path "$state" "$fleet")
  before=$(fm_backend_herdr_projection_focus_snapshot "$session") || {
    echo "warning: Herdr fleet '$fleet' could not capture exact active workspace and tab; using the ordinary flat layout" >&2
    return 2
  }
  token=$(fm_backend_herdr_fleet_record_create_pending "$state" "$fleet") || return $?
  label=$(fm_backend_herdr_fleet_workspace_label "$fleet" "$token")
  if out=$(fm_backend_herdr_cli "$session" workspace create --cwd "$cwd" --label "$label" --no-focus 2>/dev/null); then
    :
  else
    fm_backend_herdr_projection_focus_restore "$session" "$before" "fleet workspace create" || true
    echo "warning: Herdr fleet '$fleet' workspace create failed ambiguously; leaving its pending record quarantined and spawning flat" >&2
    return 2
  fi
  fm_backend_herdr_projection_focus_restore "$session" "$before" "fleet workspace create" || {
    echo "warning: Herdr fleet '$fleet' workspace create did not preserve exact focus; leaving its pending record quarantined and spawning flat" >&2
    return 2
  }
  ...
  if [ -z "$workspace" ] || [ -z "$tab" ] || [ -z "$pane" ]; then
    echo "warning: Herdr fleet '$fleet' workspace create returned incomplete IDs; leaving its pending record quarantined and spawning flat" >&2
    return 2
  fi
  if ! fm_backend_herdr_fleet_record_activate "$record" "$fleet" "$token" "$session" "$workspace" "$tab"; then
    echo "warning: Herdr fleet '$fleet' could not publish exact workspace ownership; leaving the workspace untouched and spawning flat" >&2
    return 2
  fi
  fm_backend_herdr_cli "$session" tab rename "$tab" "fleet-$fleet" >/dev/null 2>&1 || \
    echo "warning: Herdr fleet '$fleet' tab could not be renamed; continuing with the exact recorded workspace" >&2
  # shellcheck disable=SC2034  # same-process spawn caller consumes this exact ID
  FM_BACKEND_HERDR_FLEET_SESSION=$session
  # shellcheck disable=SC2034  # same-process spawn caller consumes this exact ID
  FM_BACKEND_HERDR_FLEET_WORKSPACE_ID=$workspace
  # shellcheck disable=SC2034  # same-process spawn caller consumes this exact ID
  FM_BACKEND_HERDR_FLEET_TAB_ID=$tab
  # shellcheck disable=SC2034  # same-process spawn caller consumes this exact ID
  FM_BACKEND_HERDR_FLEET_PANE_ID=$pane
}

fm_backend_herdr_fleet_join() {  # <record> <fleet-name> <cwd>
  local record=$1 fleet=$2 cwd=$3 session workspace tab token panes anchor layout choice target direction before out pane
  fm_backend_herdr_fleet_record_read "$record" "$fleet" || {
    echo "warning: Herdr fleet '$fleet' record is not an exact active record; using the ordinary flat layout" >&2
    return 2
  }
  session=$FM_BACKEND_HERDR_FLEET_RECORD_SESSION
  workspace=$FM_BACKEND_HERDR_FLEET_RECORD_WORKSPACE_ID
  tab=$FM_BACKEND_HERDR_FLEET_RECORD_TAB_ID
  token=$FM_BACKEND_HERDR_FLEET_RECORD_TOKEN
  [ "$session" = "$(fm_backend_herdr_session)" ] || {
    echo "warning: Herdr fleet '$fleet' belongs to a different named session; refusing to adopt it and spawning flat" >&2
    return 2
  }
  fm_backend_herdr_fleet_workspace_matches_record "$session" "$workspace" "$tab" "$fleet" "$token" || {
    echo "warning: Herdr fleet '$fleet' workspace or tab no longer matches its durable record; refusing reuse and spawning flat" >&2
    return 2
  }
  panes=$(fm_backend_herdr_cli "$session" pane list --workspace "$workspace" 2>/dev/null) || {
    echo "warning: Herdr fleet '$fleet' panes could not be listed; refusing reuse and spawning flat" >&2
    return 2
  }
  if ! printf '%s' "$panes" | jq -e --arg tab "$tab" '
    (.result.panes | type) == "array"
    and (.result.panes | length) > 0
    and all(.result.panes[]; .tab_id == $tab)
  ' >/dev/null 2>&1; then
    echo "warning: Herdr fleet '$fleet' has an ambiguous pane layout; refusing reuse and spawning flat" >&2
    return 2
  fi
  anchor=$(printf '%s' "$panes" | jq -r '.result.panes[0].pane_id // empty' 2>/dev/null)
  [ -n "$anchor" ] || return 2
  layout=$(fm_backend_herdr_cli "$session" pane layout --pane "$anchor" 2>/dev/null) || {
    echo "warning: Herdr fleet '$fleet' layout could not be inspected; refusing reuse and spawning flat" >&2
    return 2
  }
  choice=$(printf '%s' "$layout" | jq -r --arg workspace "$workspace" --arg tab "$tab" '
    .result.layout
    | select(.workspace_id == $workspace and .tab_id == $tab)
    | [.panes[] | select((.pane_id | type) == "string" and (.rect.width | type) == "number" and (.rect.height | type) == "number")]
    | select(length > 0)
    | max_by(.rect.width * .rect.height)
    | [.pane_id, (if .rect.width >= (.rect.height * 2) then "right" else "down" end)]
    | @tsv
  ' 2>/dev/null) || choice=
  [ -n "$choice" ] || {
    echo "warning: Herdr fleet '$fleet' layout response was ambiguous; refusing reuse and spawning flat" >&2
    return 2
  }
  target=${choice%%$'\t'*}
  direction=${choice#*$'\t'}
  [ -n "$target" ] && { [ "$direction" = right ] || [ "$direction" = down ]; } || return 2
  before=$(fm_backend_herdr_projection_focus_snapshot "$session") || {
    echo "warning: Herdr fleet '$fleet' split could not capture exact active workspace and tab; refusing the split and spawning flat" >&2
    return 2
  }
  if out=$(fm_backend_herdr_cli "$session" pane split "$target" --direction "$direction" --ratio 0.5 --cwd "$cwd" --no-focus 2>/dev/null); then
    :
  else
    fm_backend_herdr_projection_focus_restore "$session" "$before" "fleet pane split" || true
    fm_backend_herdr_fleet_record_mark_ambiguous "$record" "$fleet" || true
    echo "warning: Herdr fleet '$fleet' split failed ambiguously; quarantining the fleet record and spawning flat" >&2
    return 2
  fi
  fm_backend_herdr_projection_focus_restore "$session" "$before" "fleet pane split" || {
    fm_backend_herdr_fleet_record_mark_ambiguous "$record" "$fleet" || true
    echo "warning: Herdr fleet '$fleet' split did not preserve exact focus; quarantining the fleet record and spawning flat" >&2
    return 2
  }
  pane=$(printf '%s' "$out" | jq -r --arg workspace "$workspace" --arg tab "$tab" '
    .result.pane
    | select(.workspace_id == $workspace and .tab_id == $tab)
    | .pane_id // empty
  ' 2>/dev/null)
  if [ -z "$pane" ]; then
    fm_backend_herdr_fleet_record_mark_ambiguous "$record" "$fleet" || true
    echo "warning: Herdr fleet '$fleet' split returned incomplete IDs; quarantining the fleet record and spawning flat" >&2
    return 2
  fi
  # shellcheck disable=SC2034  # same-process spawn caller consumes this exact ID
  FM_BACKEND_HERDR_FLEET_SESSION=$session
...
# Create the first fleet workspace or join its exact active durable record.
# Return 2 requests the caller's ordinary flat fallback; return 1 is a backend
# prerequisite failure that remains terminal for the selected Herdr backend.
fm_backend_herdr_fleet_create_or_join() {  # <state-dir> <fleet-name> <cwd>
  local state=$1 fleet=$2 cwd=$3 record
  fm_backend_herdr_version_check || return 1
  fm_backend_herdr_server_ensure "$(fm_backend_herdr_session)" || return 1
  record=$(fm_backend_herdr_fleet_record_path "$state" "$fleet")
  if [ -e "$record" ] || [ -L "$record" ]; then
    fm_backend_herdr_fleet_join "$record" "$fleet" "$cwd"
  else
    fm_backend_herdr_fleet_create "$state" "$fleet" "$cwd"
  fi
}

# Close one exact fleet pane while preserving the exact active workspace/tab.
# Closing the active tab's last pane is refused because that tab cannot then be
# restored; teardown leaves the now-agent-free pane for manual inspection.
fm_backend_herdr_fleet_close_pane_focus_preserving() {  # <session> <pane-id>
  local session=$1 pane_id=$2 before active_tab info target_pane target_tab workspace panes count close_status
  before=$(fm_backend_herdr_projection_focus_snapshot "$session") || {
    echo "warning: Herdr fleet cleanup could not capture exact active workspace and tab; refusing a focus-unsafe pane close" >&2
    return 1
  }
  active_tab=${before#*$'\t'}
  info=$(fm_backend_herdr_cli "$session" pane get "$pane_id" 2>/dev/null) || {
    echo "warning: Herdr fleet cleanup could not verify the exact pane" >&2
    return 1
  }
  ...
  [ "$target_pane" = "$pane_id" ] && [ -n "$target_tab" ] && [ -n "$workspace" ] || return 1
  panes=$(fm_backend_herdr_cli "$session" pane list --workspace "$workspace" 2>/dev/null) || return 1
  count=$(printf '%s' "$panes" | jq -r --arg tab "$target_tab" '[.result.panes[]? | select(.tab_id == $tab)] | length' 2>/dev/null)
  case "$count" in ''|*[!0-9]*) return 1 ;; esac
  if [ "$target_tab" = "$active_tab" ] && [ "$count" -le 1 ]; then
    echo "warning: Herdr fleet cleanup target is the captain's active last pane; leaving it instead of stealing the active tab" >&2
    return 1
  fi
  ...
  fm_backend_herdr_projection_focus_restore "$session" "$before" "fleet pane close" || true
  return "$close_status"
}

fm_backend_herdr_fleet_prune_recorded_workspace() {  # <record> <fleet> <session> <workspace-id> <tab-id>
  local record=$1 fleet=$2 session=$3 workspace=$4 tab=$5 panes workspace_out code before active_workspace
  fm_backend_herdr_fleet_record_read "$record" "$fleet" || return 1
  [ "$FM_BACKEND_HERDR_FLEET_RECORD_SESSION" = "$session" ] \
    && [ "$FM_BACKEND_HERDR_FLEET_RECORD_WORKSPACE_ID" = "$workspace" ] \
    && [ "$FM_BACKEND_HERDR_FLEET_RECORD_TAB_ID" = "$tab" ] || return 1
  workspace_out=$(fm_backend_herdr_cli "$session" workspace get "$workspace" 2>&1)
  code=$(printf '%s' "$workspace_out" | jq -r '.error.code // empty' 2>/dev/null)
  if [ "$code" = workspace_not_found ]; then
    rm -f "$record"
    return 0
  fi
  if ! printf '%s' "$workspace_out" | jq -e \
    --arg workspace "$workspace" \
    --arg expected "$(fm_backend_herdr_fleet_workspace_label "$fleet" "$FM_BACKEND_HERDR_FLEET_RECORD_TOKEN")" '
      .result.workspace.workspace_id == $workspace and .result.workspace.label == $expected
    ' >/dev/null 2>&1; then
    echo "warning: Herdr fleet '$fleet' no longer matches its durable workspace record; refusing final workspace cleanup" >&2
    return 1
  fi
  panes=$(fm_backend_herdr_cli "$session" pane list --workspace "$workspace" 2>/dev/null) || return 1
  if ! printf '%s' "$panes" | jq -e '(.result.panes | type) == "array" and (.result.panes | length) == 0' >/dev/null 2>&1; then
    echo "warning: Herdr fleet '$fleet' still contains unowned panes; refusing final workspace cleanup" >&2
    return 1
  fi
  before=$(fm_backend_herdr_projection_focus_snapshot "$session") || return 1
  active_workspace=${before%%$'\t'*}
  if [ "$active_workspace" = "$workspace" ]; then
    echo "warning: Herdr fleet '$fleet' workspace is active; refusing cleanup that cannot restore the captain's tab" >&2
    return 1
  fi
  fm_backend_herdr_cli "$session" workspace close "$workspace" >/dev/null 2>&1 || return 1
  fm_backend_herdr_projection_focus_restore "$session" "$before" "fleet workspace close" || true
  workspace_out=$(fm_backend_herdr_cli "$session" workspace get "$workspace" 2>&1)
  code=$(printf '%s' "$workspace_out" | jq -r '.error.code // empty' 2>/dev/null)
  [ "$code" = workspace_not_found ] || {
    echo "warning: Herdr fleet '$fleet' workspace close could not be confirmed; retaining its durable record" >&2
    return 1
  }
  rm -f "$record"
}
