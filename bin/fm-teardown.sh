...
if teardown_owns_worktree && [ -d "$WT" ] && [ "$FORCE" != "--force" ] && [ "$ALLOC_RECORDS_ONLY" != 1 ]; then
...
# Both are scoped to the worktree, and under records-only retirement or a
# reassigned pool slot that worktree may already belong to the other record -
# concluding its run or killing the processes inside it would reach straight
# into that task's live work, which is the exact hazard both guards exist to
# prevent. Either verdict alone skips the worktree half; the per-task temp
# root is still this task's own and is reaped regardless.
if [ "$KIND" != secondmate ] && teardown_owns_worktree && [ "$ALLOC_RECORDS_ONLY" != 1 ]; then
