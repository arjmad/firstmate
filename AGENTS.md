...
This repo is a shared template; `docs/configuration.md` owns the private gitignored surface, which must never be committed.
Ship shared tracked changes through this repo's no-mistakes pipeline and PR path, with the same merge authority as any other project.
Never add an agent name as a commit co-author.

## 2. Layout and state

`docs/configuration.md` is the single owner of the top-level operational-home layout, root resolution, and configuration schemas; each producing script's header and help own exact child fields and mutation mechanics.
Use that reference for the tracked/private boundary and every `FM_HOME` directory.
`bin/fm-send.sh` deliberately requires an explicit `FM_HOME`, so a steer cannot silently resolve against another home.
...
That skill is the single owner of verified adapter knowledge, unverified-adapter handling, and the generic effort policy.
Never dispatch on an unverified adapter; if configuration names one, use firstmate's own verified runtime for current work and ask only whether to verify the requested runtime before future use.
...
Do not add model-specific versions of the generic effort policy.
