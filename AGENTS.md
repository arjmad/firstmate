...
config/herdr-presentation-spaces  optional "on" opt-in to, or explicit "off" opt-out from, Herdr's disposable single-task visual projection, which stays off for an unconfigured home; LOCAL, gitignored; inherited by secondmate homes; see docs/herdr-backend.md "Presentation spaces"
...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, and `kimi`, plus `muse` and experimental `prime-agent` for crewmates and scouts only; never dispatch on an unverified adapter.
