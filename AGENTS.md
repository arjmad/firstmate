...
config/google-workspace  Google account table and Bitwarden coordinates for bin/fm-google-workspace.sh ("account <slug> <email>" rows plus bws_project_id and bws_secret_key); LOCAL, gitignored, and not inherited; no built-in default, so a missing or malformed file refuses the tooling; see docs/configuration.md "Google Workspace accounts"
...
state/               runtime records and signals; gitignored
  ...
  <id>.progress      touched for observed native-harness activity inside one turn (Pi native progress, Claude PostToolUse); bin/fm-busy-event.sh owns its generation binding and bin/fm-watch.sh reads it beside turn-ended for the busy-age bound only, never as a completed turn
