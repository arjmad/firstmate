...
test_existing_launch_templates_are_byte_pinned() {
  # The claude template carries this fork's local model-endpoint routing:
  # __ENDPOINTENV__ prefixes the proxy env and __STRICTMCP__/__MCPCONFIG__ append
  # the MCP flags, all empty on a stock Anthropic launch. The pin is still exact,
  # so an accidental change to the template still fails here.
  assert_source_line "    claude) printf '%s' '__ENDPOINTENV__CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false claude --dangerously-skip-permissions __MODELFLAG____EFFORTFLAG____STRICTMCP____MCPCONFIG__\"\$(__OPINPUT__ encode launch-brief < __BRIEF__)\"' ;;"
