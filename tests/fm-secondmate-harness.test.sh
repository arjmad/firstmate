...
#      config/model-endpoints.json, config/crew-harness, config/backlog-backend,
#      and config/herdr-presentation-spaces - down into each secondmate home's
#      config/, so the secondmate's OWN crewmates, dispatch profiles, model
#      endpoints, backlog backend, and Herdr presentation opt-in inherit
...
test_propagate_lib() {
  ...
  rm -f "$src/crew-dispatch.json" "$src/model-endpoints.json" "$src/crew-harness" "$src/backlog-backend" "$src/herdr-presentation-spaces"
  propagate_inheritable_config "$src" "$dest"
