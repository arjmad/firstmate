...
test_registry_rule_reaches_every_ship_mode_once() {
  local home id proj mode id_proj_mode brief
  home="$TMP_ROOT/registry-rule-home"
  write_registry "$home"

  for id_proj_mode in "brief-reg-nm:no-registry-proj:no-mistakes" \
                      "brief-reg-dpr:direct-proj:direct-PR" \
                      "brief-reg-loc:local-proj:local-only"; do
    id=${id_proj_mode%%:*}
    proj=${id_proj_mode#*:}; proj=${proj%%:*}
    mode=${id_proj_mode##*:}
    FM_HOME="$home" FM_CLASSIFY_PAUSED_VERB=paused \
      "$ROOT/bin/fm-brief.sh" "$id" "$proj" --mode "$mode" >/dev/null 2>&1
    brief="$home/data/$id/brief.md"
    assert_present "$brief" "$id: brief was not scaffolded"
  ...
  FM_HOME="$home" FM_CLASSIFY_PAUSED_VERB=paused \
    "$ROOT/bin/fm-brief.sh" brief-reg-lab local-proj --mode local-only --herdr-lab >/dev/null 2>&1
