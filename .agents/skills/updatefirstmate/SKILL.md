...
Firstmate is its own repo, behind the same no-mistakes gate as any project, so new tracked material (`AGENTS.md`, `.claude/settings.json`, `bin/`, `.agents/skills/`, and public `skills/`) reaches `main` and then sits there until each running firstmate pulls it.
Only `AGENTS.md`, `.claude/settings.json`, `bin/`, and `.agents/skills/` are loaded by a running firstmate or its harness; public `skills/` is installer-facing and is not loaded by firstmate.
...
A tracked-files fast-forward leaves every private operational path owned by `docs/configuration.md` untouched, so a secondmate's in-flight work is never disrupted.
...
2. **Re-read AGENTS.md if your own instructions changed.**
   When the updater printed `reread-firstmate: yes`, the watched instruction surface (`AGENTS.md`, `bin/`, or `.agents/skills/`) just advanced under you.
...
- **Secondmates are never disrupted.**
  A secondmate gets a tracked-files fast-forward that leaves the private surface owned by `docs/configuration.md` untouched, plus a gentle re-read nudge.
