...
`bin/fm-spawn.sh --reuse-worktree <path>` is the supported relaunch spawn for that case; its header owns the preconditions, and it refuses on its own when the recorded owner is not confidently dead.
