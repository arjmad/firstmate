...
The firstmate PRIMARY's own `.claude/settings.json` registers two Stop hooks, dispatched auto-arm first: the Stop-owned auto-arm `bin/fm-claude-stop-autoarm.sh` (`asyncRewake: true`, `timeout: 28800`) and then `bin/fm-turnend-guard.sh --claude`; that order is load-bearing (a synchronous guard exit 2 stops dispatch of later Stop hooks, so the auto-arm must run before the guard can block), and exiting the guard with status 2 plus stderr reliably forces the model to continue.
